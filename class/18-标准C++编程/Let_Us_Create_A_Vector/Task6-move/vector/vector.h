...
public:
...
#ifdef MOVE
    vector(vector<ValueType> &&other);    // move constructor
    vector<ValueType> &operator=(vector<ValueType> &&other);  // move assginment
#endif
    ...
    // for testing move assignment
    vector<ValueType> operator+(const vector<ValueType> &other) const;
...
#ifdef MOVE
template <typename ValueType>
vector<ValueType>::vector(vector<ValueType>&& other){
    _capacity = std::move(other._capacity);
    _count = std::move(other._count);
    _elements = std::move(other._elements);
    other._elements = nullptr; // don't forget it!
