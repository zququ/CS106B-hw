...
class Timer {
public:
    void start() { current = chrono::high_resolution_clock::now(); }

    void stop() { total += chrono::high_resolution_clock::now() - current; }

    double elapsed() const { return chrono::duration_cast<chrono::nanoseconds>(total).count(); }
...
struct StreamDiverter {
    ...
    streambuf *oldBuf;
    StreamDiverter() { oldBuf = cout.rdbuf(buffer.rdbuf()); }

    ~StreamDiverter() { cout.rdbuf(oldBuf); }
};
...
void PerformanceGUI::startTests(const TimeTest &test) {
    ...
    auto onNewK = [test, colors, this](int k) {
        ...
        if (test.ks.size() != 1) {
            console->doWithStyle(colors.at(k), GColorConsole::BOLD, [&] { *console << "k = " << k << endl; });
        }
    };
...
void PerformanceGUI::repaint() {
    ...
    GRectangle header = {kPadding, kPadding, window().getCanvasWidth() - 2 * kPadding, kHeaderHeight};

    double chartTop = header.y + header.height;

    /* Set up the chart. */
    GRectangle chart = {kPadding,
                        chartTop,
                        window().getCanvasWidth() - 2 * kPadding,
                        window().getCanvasHeight() - chartTop - kPadding};

    /* And the legend. */
    GRectangle legend = {chart.x + kLegendXOffset, chart.y + kLegendYOffset, kLegendWidth, kLegendHeight};
...
void PerformanceGUI::drawChart(GWindow &window, const GRectangle &bounds) {
    ...
    /* Draw it! */
    LineGraphRender::construct(
        lines, axisLabels, yLabels, axisTicks, 0, bounds, kAxisFont, kAxisFont, kLineColors, kAxisColor)
        ->draw(window);
}

void PerformanceGUI::drawLegend(GWindow &window, const GRectangle &bounds) {
    /* If there is only one value of k, don't draw anything. */
    if (currTest.ks.size() <= 1)
        return;
