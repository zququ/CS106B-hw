...
/* Graphics constants. */
const double kEventTimerSpeed
    = 1;  // Fast enough for things to run smoothly, not enough to drown us
...
const size_t kMaxAxisLabels
    = 10;  // Max number of major tick marks on the X axis

/* Content areas. */
const double kHeaderHeight = 50;
const Font kHeaderFont(FontFamily::SERIF, FontStyle::BOLD_ITALIC, 24,
                       kAxisColor);
...
const Font kLegendFont(FontFamily::SANS_SERIF, FontStyle::NORMAL, 12,
                       kAxisColor);
...
struct TimeTest {
    function<void(size_t n, size_t k, Timer &timer)>
        callback;           // Given n, k, and a timer, do work.
...
const vector<TimeTest> kAllTests = {
    ...
    {[](int n, int k, Timer &timer) {
     ...
     linRange(0, 10000, 1000), expRange(1, 10, 2), "selectionSort"},
    ...
    {[](int n, int k, Timer &timer) {
     ...
     linRange(0, 10000, 1000), expRange(1, 10, 2), "insertionSort"},
    ...
    {[](int n, int k, Timer &timer) {
     ...
     linRange(0, 40000, 2000), expRange(1, 10, 2), "mergeSort"}};

/* Test driver, factored out from any particular method of displaying the test
 * results. */
class TestDriver {
public:
    TestDriver(const TimeTest &test, function<void(int k)> onNewK,
               function<void(int n, double value)> onNewN,
               function<void()> onStop)
        : test(test), onNewK(onNewK), onNewN(onNewN), onStop(onStop) {
    }
...
private:
    ...
    vector<string> axisLabels{
        2};  // Labels to use on the x axis. Defaults to two blank strings
...
void PerformanceGUI::startTests(const TimeTest &test) {
    ...
    auto onNewN = [this](int n, double elapsed) {
        ...
        *console << "  n = " << left << setw(10) << (to_string(n) + ": ")
                 << elapsed / 1e6 << "ms" << endl;
    };
     ...
     * to use. For example, if we want 11 labels and the max is 10, we'll
     * compute ceil(11 / 10) - 1 = 2, and have one minor tick in between each
     * set of major ticks.
     *
     * We actually compute this using the technique that (a + b - 1) / b
     * computes ceil(a / b).
     */
    ...
    for (size_t i = 0; i < numMajor; i++) {
        size_t effectiveIndex
            = i * axisTicks;  // We're skipping things, after all!
    ...
    /* The axisTicks parameter actually literally controls how many minor ticks
     * are drawn, not the skip size between major ticks. As a result, we need to
     * subtract one here in order to ensure that we draw teh right number of
     * minor ticks.
     */
    axisTicks--;
    ...
    console->doWithStyle(FontSize(16), [&] { *console << test.name << endl; });
}
...
void PerformanceGUI::repaint() {
    ...
    GRectangle header
        = {kPadding, kPadding, window().getCanvasWidth() - 2 * kPadding,
           kHeaderHeight};
    ...
    /* Set up the chart. */
    GRectangle chart
        = {kPadding, chartTop, window().getCanvasWidth() - 2 * kPadding,
           window().getCanvasHeight() - chartTop - kPadding};

    /* And the legend. */
    GRectangle legend = {chart.x + kLegendXOffset, chart.y + kLegendYOffset,
...
void PerformanceGUI::drawChart(GWindow &window, const GRectangle &bounds) {
    /* Y-axis labels will be empty. We need a minimum of two, though, so we'll
     * make those. */
    ...
    for (const auto &result : results) {
        ...
        for (int i = 0; i < result.size(); i++) {
            line.push_back({i * 1.0 / (currTest.ns.size() - 1),
    ...
    /* Draw it! */
    LineGraphRender::construct(lines, axisLabels, yLabels, axisTicks, 0, bounds,
                               kAxisFont, kAxisFont, kLineColors, kAxisColor)
        ->draw(window);
}
...
void PerformanceGUI::drawLegend(GWindow &window, const GRectangle &bounds) {
    ...
    /* Otherwise, there are many values of k. Assemble labels based on the
     * number of lines we have drawn so far.
     */
    vector<string> labels;
    for (size_t i = 0; i < currTest.ks.size() && i < size_t(results.size());
         i++) {
        labels.push_back("k = " + to_string(currTest.ks[i]));
    }

    /* Set up the legend. Don't draw it yet; we need to clear the area behind
     * it. */
    auto legend = LegendRender::construct(labels, kLineColors, bounds,
                                          kLegendFont, kLegendFont.color());
...
CONSOLE_HANDLER("Time Tests") {
    ...
    do {
        int choice
            = makeSelectionFrom("Choose a function to time-test:", options);
        ...
        auto onNextN = [&](int n, double result) {
            cout << "  n = " << left << setw(10) << (to_string(n) + ": ")
                 << result / 1e6 << "ms" << endl;
        };
