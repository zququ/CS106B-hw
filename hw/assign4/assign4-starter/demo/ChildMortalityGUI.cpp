...
namespace {
    ...
    private:
        ...
        using Scorer = std::function<double (const std::string&)>;
    ...
    void ChildMortalityGUI::actionPerformed(GObservable* source) {
        if (source == mLowest) {
            ...
            chooseBestCountriesBy([this] (const string& country) {
                return -mData[country][mData[country].size()-1];
            }, [] (const DataPoint& point) {
                ostringstream builder;
                builder << point.label << " (" << -point.priority << ")";
                return builder.str();
            });
            ...
            chooseBestCountriesBy([this] (const string& country) {
                return mData[country][0] - mData[country][mData[country].size()-1];
            }, [] (const DataPoint& point) {
                ostringstream builder;
                builder << point.label << " (-" << point.priority  << ")";
                return builder.str();
            ...
            /* Measure in the revesre direction to ensure we find the most improved. */
            chooseBestCountriesBy([this] (const string& country) {
                double now  = mData[country][mData[country].size()-1];
                double then = mData[country][0];
                return (then - now) / then;
            }, [this] (const DataPoint& point) {
                double now  = mData[point.label][mData[point.label].size() -1];
                double then = mData[point.label][0];
                double improvement = 100.0 * (then - now) / then;

                ostringstream builder;
                builder << point.label << " (-" << fixed << setprecision(2) << improvement << "%)";
                return builder.str();
            });
    ...
    void ChildMortalityGUI::chooseBestCountriesBy(Scorer scorer, Formatter formatter) {
        ...
        for (auto entry: mCountries) {
            if (!mData.containsKey(entry.label)) {
                ostringstream out;
    ...
    void ChildMortalityGUI::drawPlot(GWindow& window, const GRectangle& bounds) const {
        ...
        for (const auto& country: mCountries) {
            vector<GPoint> line;
            for (int i = 0; i < mData[country.label].size(); i++) {
                /* Coordinates should be in [0, 0] x [1, 1]. */
                double x = double(i) / (mData[country.label].size() - 1);
                double y = mData[country.label][i] / kMaxYValue;
                line.push_back({ x, y });
            }
